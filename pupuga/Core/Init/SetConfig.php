...
class SetConfig extends Pupuga\Config
{
    ...
    public function registerPostTypesTaxonomies()
    {
        ...
        add_action('pre_get_posts', array($this, 'setSortAction'));
        add_action('pre_get_posts', array($this, 'removeSlugFromLink') );
    }

    public function setSortAction($query): void
    {
        if(is_admin()) {
            switch (strval($query->query['orderby'])) {
                case 'title':
            ...
            $query->query['order'] = $query->query['order'] ?? 'desc';
